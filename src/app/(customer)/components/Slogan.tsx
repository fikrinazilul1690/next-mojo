...
export default function Slogan({ Icon, title, body }: Params) {
  ...
  return (
    <div>
      <div className='w-[90px] h-[90px] rounded-full border border-separate ml-0 my-auto flex items-center justify-center'>
        {icon}
      </div>
      <h2 className='text-xl font-bold'>{title}</h2>
      <p className='break-words sm:w-[225px] w-[150px] text-sm'>{body}</p>
    </div>
  );
}
